...
export class AdminService {
  ...
  getAdminInformation(par?: SearchAdminParams): Observable<any> {
    ...
    if (par) {
      ...
      if (par.id) {
        // console.log(par.id);
        return this.http.get(baseUrl + 'api/admininfosum', {params: {id: par.id}}).pipe(
          map(res => res.json())
        );
      /*分页查询*/
      } else if (par.page) {
        return this.http.get(baseUrl + 'api/admininfosum', {params: {page: par.page}}).pipe(
          map(res => res.json())
        );
        /*条件判断查询*/
      } else if (par.name || par.code || par.age || par.reach || par.data) {
        if (!par.code && !par.name && !par.data && Number(par.age) === -1 && Number(par.reach) === - 1 ) {
          return this.http.get(baseUrl + 'api/admininfosum', {params: {page: 1}}).pipe(
            map(res => res.json())
          );
        } else {
          return this.http.get(baseUrl + 'api/admininfosum', {params: {name : par.name, code: par.code, age: par.age, reach: par.reach, data: par.data
            }}).pipe(
            map(res => res.json()
            ));
        }
  ...
  deleteAdminInformation(par: SearchAdminParams): Observable<any> {
    ...
    if (par) {
      console.log(par);
      return this.http.get(baseUrl + 'api/admindelete', {params: {id: par.id}}).pipe(
        map(res => res.json())
      );
    }
  }
  getdetailInformation(id): Observable<EditAdminIformation> {
    if (id) {
     return this.http.get(baseUrl + 'api/admindetail', {params: {id: id}}).pipe(
        map(res => res.json())
      );
    }
  }
  ...
  editAdminInformation(par: EditAdminIformation, id: number): Observable<any> {
    if (par) {
      par.id = id;
      return  this.http.post(baseUrl +'api/adminedit', {params: par}).pipe(
        map(res => res.json())
      );
    }
}
  ...
  addAdminInformation(par: EditAdminIformation): Observable<any> {
    // console.log(par);
    if (par) {
      return this.http.post(baseUrl +'api/addadmin', {params: par}).pipe(
        map(res => res.json()
        )
      );
    }
  ...
  regAdminRoot(id: number, root: number): Observable<any> {
    if (id && root) {
     return this.http.post(baseUrl +'api/adminroot', {params: {id: id, root: root}}).pipe(
        map(res => res.json()
        )
      );
    } else {
